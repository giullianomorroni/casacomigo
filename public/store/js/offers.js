...
$(document).ready(function() {
	...
	$.ajax({
		  url: (PROTOCOL+"//"+HOST)+"/oferta/listar/limite/12/pular/0",
...
function mountProducts(data) {
	...
	for (var d in data) {	
		...
		var html = 
			...
			"		<h4>de R$ "+produto.PrecoOriginal+" </h4> "+
			"		<h3>POR R$ "+produto.Preco+" </h3> "+
		...
		var html = 
			...
			"				<a class='btn btn-primary' href='#'> de R$ " +produto.Preco+ " POR R$ " +produto.Preco+ "</a>"+
			"			</h4> "+
			"		</div> "+
			"	</div> "+
			"</li>";
